...
class vis_Resize(object):
    ...
    def __call__(self, image, label):
        ...
        image_crop = cv2.resize(image, dsize=(test_size, test_size), interpolation=cv2.INTER_LINEAR)
        ...
        #new_h, new_w = test_size, test_size
        s_mask = cv2.resize(s_mask.astype(np.float32), dsize=(test_size, test_size),interpolation=cv2.INTER_NEAREST)
